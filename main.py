...
class GroupMemberPlugin(Star):
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        # 0. 优先检查管理员修正确认（仅对非命令消息检测）
        msg_lower = message_text.strip().lower()
        is_command = msg_lower.startswith(('/qy_', 'qy_', '/qy', 'qy'))
        if not is_command:
            session_key = f"{group_id}_{sender_id}"
            if session_key in self._admin_handler._pending_confirmations:
                result = await self._admin_handler.process_admin_command(event, group_id, message_text)
